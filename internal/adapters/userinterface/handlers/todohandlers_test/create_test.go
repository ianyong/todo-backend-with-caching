...
var createdTodo = todoviews.View{
	ID:          1,
	Name:        "Todo",
	Description: "This is a valid todo.",
...
func (s *CreateTodoTestSuite) TestCreateTodo() {
	request, err := http.NewRequest(http.MethodPost, "/api/v1/todos", strings.NewReader(validCreateJSON))
	if err != nil {
...
const missingFieldCreateJSON = `{
    "name": "Todo",
    "dueDate": "2021-11-01T00:00:00.000000+00:00"
}`

func (s *CreateTodoTestSuite) TestCreateTodoMissingField() {
	request, err := http.NewRequest(http.MethodPost, "/api/v1/todos", strings.NewReader(missingFieldCreateJSON))
	if err != nil {
		s.T().Errorf("Error creating request: %v", err)
	}
...
func (s *CreateTodoTestSuite) TestCreateTodoExtraField() {
	request, err := http.NewRequest(http.MethodPost, "/api/v1/todos", strings.NewReader(extraFieldCreateJSON))
	if err != nil {
		s.T().Errorf("Error creating request: %v", err)
	}
